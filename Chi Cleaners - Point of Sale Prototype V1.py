...
if not os.path.isdir("logs"):
    os.mkdir("logs")
log_path = f"logs\\LOG [{start_time}].csv"
# Add log titles
...
if not os.path.isdir("output"):
    log(get_line_number(), "info", "does not exist", '"output" directory will now be created...')
    # Creates directory if it doesn't exist
    os.mkdir("output")
else:
    log(get_line_number(), "info", "directory exists", '"output"')
    # Does nothing if it does
# Store the paths for `receipt.txt` and `tags.txt` to make code simpler later
receipt_path = "output\\receipt.txt"
tags_path = "output\\tags.txt"

# Check if the "output\\history" directory exists
if not os.path.isdir("output\\history"):
    log(get_line_number(), "info", "does not exist", '"output\\history" directory will now be created...')
    # Creates directory if it doesn't exist
    os.mkdir("output\\history")
else:
    log(get_line_number(), "info", "directory exists", '"output\\history"')
    # Does nothing if it does
# Store the paths for `output\\history` to make code simpler later
history_path = "output\\history"

# Also check if the "products" directory exists
if not os.path.isdir("products"):
    log(get_line_number(), "info", "does not exist", '"products" directory will now be created...')
    # Creates directory if it doesn't exist
    os.mkdir("products")
else:
    log(get_line_number(), "info", "directory exists", '"products"')
# Defining path to the products CSV
products_path = "products\\products.csv"


# Check if the "config.txt" file exists
if not os.path.isfile("products\\config.txt"):
    ...
    config_file = open("products\\config.txt", "w")
...
config_file = open("products\\config.txt", "r")
...
def historic_copy(path):
    ...
    if os.path.isfile(path):
        ...
        # print(new_file_name)      # debug only
        shutil.copy(path, f"output\\history\\{new_file_name}")
...
def make_receipt(order_list):
    ...
    for item in order_list:
        ...
        receipt.write(f"\t{name}{' ' * (28 - len(name))} \t|\t {pricify(base_price)}\n")
        print(f"\t{name}{' ' * (28 - len(name))} \t|\t {pricify(base_price)}")
    ...
    receipt.write(f"Total price of items: {pricify(total_base_cost)}\n\n")
    print(f"Total number of items: {len(order_list)}")
    print(f"Total price of items: {pricify(total_base_cost)}\n")

    # Calculate discount eligibility from VAT inclusive price
    if total_base_cost < 15:
        discount = 0
    ...
    receipt.write(f"of which {pricify(vat_amount)} is VAT\n\n")
    ...
    print(f"of which {pricify(vat_amount)} is VAT\n")
